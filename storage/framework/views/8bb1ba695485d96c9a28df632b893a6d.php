...
<head>
    ...
    <title>Payslip - <?php echo e($employeeFieldValues['employee_name']); ?></title>
    <style>
        * { box-sizing: border-box; }
        ...
        body {
            margin: 0;
            padding: 18px;
            background: <?php echo e($renderMode === 'pdf' ? '#ffffff' : '#eef1f2'); ?>;
            color: #172022;
...
<div class="container" id="payslip-content">
    <div class="header">
        <?php if(($templateConfig['logo']['show'] ?? true) && !empty($logoDataUri)): ?>
            <div class="header-logo"><img src="<?php echo e($logoDataUri); ?>" alt="Company logo"></div>
        <?php endif; ?>
        <?php if(($templateConfig['qr']['show'] ?? true) && !empty($qrCodeDataUri)): ?>
            <div class="header-qr">
                <img src="<?php echo e($qrCodeDataUri); ?>" alt="ESS portal QR code">
                <div class="header-qr-label">ESS Portal</div>
            </div>
        <?php endif; ?>
        <div class="company-name"><?php echo e($companyName); ?></div>
        <?php if(!empty($companySettings['companyAddress'])): ?>
            <div class="company-address"><?php echo e($companySettings['companyAddress']); ?></div>
        <?php endif; ?>
        <?php if(count($companyContacts) > 0): ?>
            <div class="company-contact"><?php echo e(implode(' | ', $companyContacts)); ?></div>
        <?php endif; ?>
        <div class="payslip-title"><?php echo e($templateConfig['header']['title'] ?? 'Salary Slip'); ?></div>
        <div class="period-title"><?php echo e($payrollEntry->payrollRun->pay_period_start->format('F Y')); ?></div>
    </div>

    <?php if(($templateConfig['sections']['employee_info'] ?? true) && count($employeeFieldRows) > 0): ?>
        <table class="employee-table">
            <tr class="section-row"><th colspan="4" class="section-header">Employee Information</th></tr>
            <?php $__currentLoopData = $employeeFieldRows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fieldRow): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr>
                    <?php $__currentLoopData = $fieldRow; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <td class="info-label"><?php echo e($field['label']); ?></td>
                        <td class="info-value"><?php echo e($employeeFieldValues[$field['key']] ?? 'N/A'); ?></td>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <?php if(count($fieldRow) === 1): ?>
    ...
    <?php if($templateConfig['sections']['leave'] ?? true): ?>
        <table>
            <tr><th colspan="4" class="section-header">Leave Details</th></tr>
            <tr>
                <td class="leave-period-label">Paid Leave</td>
                <td><?php echo e(number_format((float) ($payrollEntry->paid_leave_days ?? 0), 2)); ?></td>
                <td class="leave-period-label">Unpaid Leave</td>
                <td><?php echo e(number_format((float) ($payrollEntry->unpaid_leave_days ?? 0), 2)); ?></td>
            </tr>
            <?php if($leaveBalances->isNotEmpty()): ?>
                ...
                <?php $__currentLoopData = $leaveBalances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $balance): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr>
                        ...
                        <td class="amount"><?php echo e(number_format((float) $balance['entitled'], 2)); ?></td>
                        <td class="amount"><?php echo e(number_format((float) $balance['taken'], 2)); ?></td>
                        <td class="amount"><?php echo e(number_format((float) $balance['balance'], 2)); ?></td>
                    </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php endif; ?>
        </table>
    ...
    <?php if($templateConfig['sections']['earnings_deductions'] ?? true): ?>
        <table class="salary-table">
            ...
            <?php for($index = 0; $index < $maxRows; $index++): ?>
                <tr>
                    <td><?php echo e($earningsList[$index]['name'] ?? ''); ?></td>
                    <td class="amount"><?php echo e(isset($earningsList[$index]) ? formatCurrency($earningsList[$index]['amount']) : ''); ?></td>
                    <td><?php echo e($deductionsList[$index]['name'] ?? ''); ?></td>
                    <td class="amount"><?php echo e(isset($deductionsList[$index]) ? formatCurrency($deductionsList[$index]['amount']) : ''); ?></td>
                </tr>
            <?php endfor; ?>
            <tr class="total-row">
                <td>Total Earnings</td>
                <td class="amount"><?php echo e(formatCurrency($totalEarnings)); ?></td>
                <td>Total Deductions</td>
                <td class="amount"><?php echo e(formatCurrency($totalDeductions)); ?></td>
            </tr>
            <tr class="net-salary-row">
                <td colspan="3">NET PAY</td>
                <td class="amount"><?php echo e(formatCurrency($payrollEntry->net_pay)); ?></td>
            </tr>
        </table>
    <?php endif; ?>

    <?php if(($templateConfig['sections']['employer_contributions'] ?? true) && count($employerItems) > 0): ?>
        <table>
            <tr><th colspan="2" class="section-header">Employer Contributions (HR Record - Not Deducted from Employee)</th></tr>
            <?php $__currentLoopData = $employerItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr class="employer-row">
                    <td><?php echo e($item['name']); ?></td>
                    <td class="amount"><?php echo e(formatCurrency($item['amount'])); ?></td>
                </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </table>
    <?php endif; ?>
    ...
    <div class="footer">
        <p><strong><?php echo e($templateConfig['footer']['text'] ?? 'This is a computer-generated payslip.'); ?></strong></p>
...
<?php /**PATH /Users/haseeb/Downloads/FileSharing/fileshare-next/hrm-management-system/resources/views/payslips/template.blade.php ENDPATH**/ ?>